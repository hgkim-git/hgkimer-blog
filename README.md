...
`access token`이 만료됐을 때 서버가 자동으로 `refresh token`을 이용해 재발급을 시도한다면 사용자 경험은 좋아지지만, 모든 요청에 이를 적용하면 API
계약이 불명확해지고 디버깅도 어려워집니다.
...
"자동화된 복구는 UX와 예측 가능성 사이에서 경계를 명확히 해야 한다"는 원칙 하에, 사용자가 직접 브라우저에서 페이지를 탐색할 때만 자동 복구를 허용했습니다. API
클라이언트(JS)는 401을 받으면 별도 재시도 로직을 통해 처리합니다.
...
대시보드나 목록에서 카테고리 수를 반복적으로 읽는 읽기 중심 워크로드에서 집계 쿼리보다 성능이 유리합니다. 단, 배치 작업이나 직접 DB 수정 시 값이 틀어질 수 있어 정합성 보정
로직이 추가로 필요하다는 단점이 있습니다.
